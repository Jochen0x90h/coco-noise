 ...
 * @param x input of noise (only 16 bits of x are used, i.e. pattern repeats at 256.0)
 ...
 * @return noise function at given x
 */
inline uint8_t noiseU8(int x) {return noiseI8(x) + 128;}


/**
 * Generate noise in the range [-32768, 32767] for x with 16 bit fractional part
 * @param x input of noise (only 24 bits of x are used, i.e. pattern repeats at 256.0)
 * @return noise function at given x
 */
int noiseI16(int x);
 ...
 * @return noise function at given x
 */
inline int noiseU16(int x) {return noiseI16(x) + 32768;}
